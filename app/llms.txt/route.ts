...
export async function GET(): Promise<Response> {
  ...
  const lines: string[] = [
    `# ${SITE_NAME}`,
    "",
    "> A community index of open-source Agent Plugins: self-contained packages that can contain agent skills and MCP servers.",
    "",
    "The index is built from public GitHub repositories that declare a valid plugin.json manifest under the Agent Plugins specification. It is a discovery index, not the publisher of the listed plugins.",
    "",
    "## Canonical pages",
    "",
    ...
    "",
    `- GET ${absoluteUrl("/api/v1/plugins")} — search the index. Params: q (free text), category (keyword), type (skills|mcp), transport (stdio|streamable-http|sse), sort (stars|updated|recent), page, per_page (1-50, default 24).`,
