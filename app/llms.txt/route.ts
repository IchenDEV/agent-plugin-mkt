...
export async function GET(): Promise<Response> {
  ...
  const lines: string[] = [
    ...
    "",
    "A community index of open-source plugins for Codex, Claude Code, and the Agent Plugins v1 format, indexed from canonical GitHub manifests.",
    "",
    "## REST API (JSON, read-only, no auth)",
    "",
    "- GET /api/v1/plugins — search the index. Repeat protocol to match any selected runtime (codex|claude-code|agent-plugins); other params: q, category, type (skills|mcp), transport (stdio|streamable-http|sse), sort (stars|updated|recent), page, per_page (1-50, default 24).",
