...
const fixtureEntrySchema = z.object({
  ...
  repoForks: z.number().int().nonnegative().default(0),
  repoOpenIssues: z.number().int().nonnegative().default(0),
...
try {
  ...
  for (const [index, rawEntry] of file.plugins.entries()) {
    ...
    const result = await upsertPlugin({
      ...
      repoForks: entry.repoForks,
      repoOpenIssues: entry.repoOpenIssues,
